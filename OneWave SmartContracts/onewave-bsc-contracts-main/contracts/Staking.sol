...
contract Staking is AccessControl, ReentrancyGuard, Pausable {
    ...
    ///      Note: does not account for already-accrued unclaimed rewards (no global sum tracked).
    ///      This check prevents the most egregious insolvency scenarios.
    ...
    function _updateReward(address account) internal {
        ...
        user.pendingRewards += (user.stakedAmount * (rewardPerTokenStored - user.rewardDebt)) / 1e18;
        user.rewardDebt = rewardPerTokenStored;
    }
